...
#### **--volume**, **-v**=_source:target[:options]_
...
The value of _mapped-xattr_ specifies that 9p store symlinks and some file
...
_none_ instead, but keep in mind that the guest will not be able to do things
that the user running the virtual machine cannot do, e.g. create files owned by
another user. Using _none_ is almost certainly the best choice for read-only
volumes.

Example: `-v "$HOME/git:$HOME/git:ro,security_model=none"`

Default volume mounts are defined in _containers.conf_. Unless changed, the default values
is `$HOME:$HOME`.
